...
##   9_Sensitivity Analyses - Repeat g-computation analyses using composite of
##                            any parasites or pigment
##  
##   This script is the same as 7_Gcomputation_ActiveInfection, but replace 
##   activeHP with anyplcmal
##################################################################################
...
#   - If model with interaction anyplcmal*graviddich term does not converge,
...
overall_compute_metrics <- function(data, outcome_var, indices) {
  ...
  # 1. Fit outcome model
  # -----------------------
  # Full model with interaction between any placental infection and gravidity
  formula_full <- as.formula(
    paste(outcome_var, "~ anyplcmal*graviddich + treatmentarm + age_enroll + age2 + wealthcat_hd + bmi_enroll_hd + bmi_enroll_hd2 + GAenroll + gaenroll2 + educ_cat + male")
  )
  ...
  if (inherits(model, "try-error")) {
    formula_maineffects <- as.formula(
      paste(
        outcome_var,
        "~ anyplcmal + graviddich + treatmentarm + age_enroll + age2",
  ...
  # ------------------------------------------------------------
  # anyplcmal     : indicator of any parasites or pigment (0/1)
  # anyplcmal_sim : simulated "natural course" exposure (used as the
  #                counterfactual scenario had all women received standard-of-care 
  #                IPTp-SP)  
  #
  # We consider:
  #  - AFE (among the exposed, anyplcmal_sim == 1):
  #       margin0: risk if all exposed women were set to anyplcmal_sim = 0
  #       margin1: risk if all exposed women were set to anyplcmal_sim = 1
  #
  #  - PAF (population level):
  #       margin2: risk if everyone were set to anyplcmal = 0 
  #       margin3: risk if anyplcmal was set to its natural exposure distribution (anyplcmal_sim)
  
  # ---- AFE: subset of women with anyplcmal_sim == 1 ----
  data0 <- d %>%
    filter(anyplcmal_sim == 1) %>%
    mutate(anyplcmal=0,              # set exposure to 0 (no PM infection) for model prediction
           treatmentarm="SP")       # set treatment arm to SP for all
  data1 <- d %>%
    filter(anyplcmal_sim == 1) %>%
    mutate(anyplcmal = 1,              # set exposure to 1 (PM infection) for model prediction
           treatmentarm="SP")       # set treatment arm to SP for all
  ...
  data2 <- d %>%
    mutate(anyplcmal = 0,            # set exposure to 0 (no PM infection) for model prediction 
           treatmentarm = "SP")
  data3 <- d %>%
    mutate(anyplcmal = anyplcmal_sim, # set exposure to simulated natural course exposure distribution
           treatmentarm = "SP")
...
gravid_compute_metrics <- function(data, outcome_var, indices, grav_subgroup = NULL) {
  ...
  # -----------------------
  formula_full <- as.formula(
    paste(outcome_var, "~ anyplcmal + treatmentarm + age_enroll + age2 + wealthcat_hd + bmi_enroll_hd + bmi_enroll_hd2 + GAenroll + gaenroll2 + educ_cat + male")
  )
...
write.xlsx(finalresults, file="Results/DPSP_SensAnalysis_AnyPM_Gcomp_Boot.xlsx")
