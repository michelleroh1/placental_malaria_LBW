...
##	 7_G-computation to estimate effects of active infection on birth outcomes
##
##   This script:
##      1. Fits logistic regression models for each outcome
##      2. Performs parametric g-computation for active infection
##      3. Uses bootstrap (BCa) to obtain 95% CIs
##      4. Produces estimates for overall population, and by primigravidae and multigravidae 
##      5. Exports an Excel to export estimates
##################################################################################
...
overall_compute_metrics <- function(data, outcome_var, indices) {
  ...
  # ------------------------------------------------------------
  # activeHP     : indicator of active placental infection (0/1)
  # activeHP_sim : simulated "natural course" exposure (used as the
  #                counterfactual scenario had all women received standard-of-care 
  #                IPTp-SP)  
  #
  # We consider:
  #  - AFE (among the exposed, activeHP_sim == 1):
  #       margin0: risk if all exposed women were set to activeHP_sim = 0
  #       margin1: risk if all exposed women were set to activeHP_sim = 1
  #
  #  - PAF (population level):
  #       margin2: risk if everyone were set to activeHP_sim = 0 
  #       margin3: risk if activeHP was set to its natural exposure distribution (activeHP_sim)
...
#   Export results 
# -----------------------
write.xlsx(finalresults, file="Results/DPSP_ActInf_Gcomp_Boot.xlsx")
