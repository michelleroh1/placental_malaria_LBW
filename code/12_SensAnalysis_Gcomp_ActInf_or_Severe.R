...
##	  12_Sensitivity Analyses - Repeat g-computation analyses of using a composite  
##                              of active or severe pigment to estimate their
##                              combined contribution to LBW 
##
##  This script is the same as 7_Gcomp_ActiveInf, but use any_actsev variable
...
db <- read.dta13('Databases/Created Databases/DPSP Study - Delivery and Histopath Results_HDimputed_Sim_Gcomp_FINAL.dta', 
                 nonint.factors = TRUE) %>%
  mutate(any_actsev = case_when(
    propfibrincat == ">=30%" ~ 1, 
    activeHP == 1 ~ 1,
    propfibrincat!=1 | activeHP!=1 ~ 0), 
    any_actsev_sim = case_when(
      propfibrincat_sim == ">=30%" ~ 1, 
      activeHP_sim == 1 ~ 1,
      propfibrincat_sim!=1 | activeHP_sim!=1 ~ 0)
  )
...
#   - If model with interaction any_actsev*graviddich term does not converge,
...
overall_compute_metrics <- function(data, outcome_var, indices) {
  ...
  # 1. Fit outcome model
  # -----------------------
  # Full model with interaction between any_actsev and gravidity
  formula_full <- as.formula(
    paste(outcome_var, "~ any_actsev*graviddich + treatmentarm + age_enroll + age2 + wealthcat_hd + bmi_enroll_hd + bmi_enroll_hd2 + GAenroll + gaenroll2 + educ_cat + male")
  )
  ...
  if (inherits(model, "try-error")) {
    formula_maineffects <- as.formula(
      paste(
        outcome_var,
        "~ any_actsev + graviddich + treatmentarm + age_enroll + age2",
  ...
  # ------------------------------------------------------------
  # any_actsev     : indicator of any active infection or severe pigment (0/1)
  # any_actsev_sim : simulated "natural course" exposure (used as the
  #                counterfactual scenario had all women received standard-of-care 
  #                IPTp-SP)  
  #
  # We consider:
  #  - AFE (among the exposed, any_actsev_sim == 1):
  #       margin0: risk if all exposed women were set to any_actsev_sim = 0
  #       margin1: risk if all exposed women were set to any_actsev_sim = 1
  #
  #  - PAF (population level):
  #       margin2: risk if everyone were set to any_actsev_sim = 0 
  #       margin3: risk if any_actsev was set to its natural exposure distribution (any_actsev_sim)
  
  # ---- AFE: subset of women with anyactsev_sim == 1 ----
  data0 <- d %>%
    filter(any_actsev_sim==1) %>%
    mutate(any_actsev=0,              # set exposure to 0 (no active infection or severe pigment) for model prediction
           treatmentarm="SP")       # set treatment arm to SP for all
  data1 <- d %>%
    filter(any_actsev_sim==1) %>%
    mutate(any_actsev=1,              # set exposure to 1 (active infection or severe pigment) for model prediction
           treatmentarm="SP")       # set treatment arm to SP for all
  ...
  data2 <- d %>%
    mutate(any_actsev = 0,            # set exposure to 0 (no active infection or severe pigment) for model prediction 
           treatmentarm = "SP")
  data3 <- d %>%
    mutate(any_actsev = any_actsev_sim, # set exposure to simulated natural course exposure distribution
           treatmentarm = "SP")
...
gravid_compute_metrics <- function(data, outcome_var, indices, grav_subgroup = NULL) {
  ...
  # -----------------------
  formula_full <- as.formula(
    paste(outcome_var, "~ any_actsev + treatmentarm + age_enroll + age2 + wealthcat_hd + bmi_enroll_hd + bmi_enroll_hd2 + GAenroll + gaenroll2 + educ_cat + male")
  )
  ...
  # 2. Define counterfactual scenarios for g-computation
  # ------------------------------------------------------------
  # ---- AFE: subset of women with any_actsev_sim == 1 ----
  data0 <- d %>%
    filter(any_actsev_sim==1) %>%
    mutate(any_actsev=0,               # set exposure to 0 (no active infection or severe pigment) for model prediction
           treatmentarm="SP")        # set treatment arm to SP for all
...
write.xlsx(finalresults, file="Results/DPSP_SensAnalysis_AnyActorSev_Gcomp_Boot.xlsx")
